 	...
 	B. Set the path by doing one of the following:
		...
		* Add the MCR directory to the environment variable by issuing 
         ...
         NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_apolloh.sh on UNIX and Mac
...
3. Collect the following files, based on what you are deploying: 
	...
	All Platforms:
		* apolloh.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
        Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library installer)   
		* apolloh.exe (application created by MATLAB Compiler)
        UNIX:
		* MCRInstaller.bin (MATLAB Component Runtime library 
	 	  installer)
		* apolloh (application created by MATLAB Compiler)
		* run_apolloh.sh (shell script that can be run 
	...
	All Platforms:            
		* apolloh.so (shared library, file extension 
		  varies by platform)
		* apolloh.h (library header file)
		* apolloh.ctf (Component Technology File archive,
	...
	* apolloh.xml (documentation files)
	* apolloh.pdb (program database file - if 
	  DEBUG option selected)
	* apolloh.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* apolloh.jar (Java class files and compiled M-code)
	...
	* apolloh.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* apolloh.xla (Optionally created and placed in
